...
DATA_BLOB getBlobFromJString(JNIEnv *env, jstring str) {
    ...
    result.cbData = (DWORD) strlen(nativeString)+1;
    
    return result;
}

void freeJStringFromBlob(JNIEnv *env, jstring original, DATA_BLOB blob) {
    env->ReleaseStringUTFChars(original, (const char *) blob.pbData);
}
...
JNIEXPORT jbyteArray JNICALL Java_net_sourceforge_jdpapi_DPAPI_CryptProtectData
  (JNIEnv *env, jclass clazz, jstring key, jbyteArray entropyBytes, jboolean useLocalMachine) {
    ...
    BOOL completed = CryptProtectData(&input, L"JDPAPI", &entropy, NULL, NULL, getFlags(useLocalMachine), &output);
...
JNIEXPORT jbyteArray JNICALL Java_net_sourceforge_jdpapi_DPAPI_CryptProtectDataBinary
  (JNIEnv *env, jclass clazz, jbyteArray key, jbyteArray entropyBytes, jboolean useLocalMachine) {
	  
	DATA_BLOB output;   
    DATA_BLOB input = getBlobFromBytes(env, key);
    DATA_BLOB entropy = getBlobFromBytes(env, entropyBytes);
    
    BOOL completed = CryptProtectData(&input, L"JDPAPI", &entropy, NULL, NULL, getFlags(useLocalMachine), &output);
    
    freeBytesFromBlob(env, entropyBytes, entropy);
    freeBytesFromBlob(env, key, input);
